# Table of contents
- [Hệ Thống Tưới Tự Động Sử Dụng ESP8266 NodeMCU và Blynk](#he-thong-tuoi-tu-dong-su-dung-esp8266-nodemcu-va-blynk)
  * [1. Giới thiệu](#1-gioi-thieu)
    + [Chức năng chính:](#chuc-nang-chinh)
  * [2. Yêu cầu hệ thống](#2-yeu-cau-he-thong)
    + [Phần cứng:](#phan-cung)
    + [Phần mềm:](#phan-mem)
  * [3. Sơ đồ kết nối phần cứng](#3-so-do-ket-noi-phan-cung)
  * [4. Cài đặt phần mềm](#4-cai-dat-phan-mem)
    + [4.1. Cài đặt Arduino IDE và board ESP8266](#41-cai-dat-arduino-ide-va-board-esp8266)
      - [Bước 1: Tải và cài đặt Arduino IDE](#buoc-1-tai-va-cai-dat-arduino-ide)
      - [Bước 2: Cài đặt board ESP8266 vào Arduino IDE](#buoc-2-cai-dat-board-esp8266-vao-arduino-ide)
    + [4.2. Cài đặt thư viện cho Arduino IDE](#42-cai-dat-thu-vien-cho-arduino-ide)
      - [Bước 1: Cài thư viện ESP8266WiFi](#buoc-1-cai-thu-vien-esp8266wifi)
      - [Bước 2: Cài thư viện Blynk (v0.6.1)](#buoc-2-cai-thu-vien-blynk-v061)
      - [Bước 3: Cài thư viện Time và NTPClient](#buoc-3-cai-thu-vien-time-va-ntpclient)
  * [5. Cài đặt Blynk App (Legacy)](#5-cai-dat-blynk-app-legacy)
    + [5.1. Tải và cài đặt Blynk App (Legacy)](#51-tai-va-cai-dat-blynk-app-legacy)
    + [5.2. Đăng ký tài khoản và đăng nhập](#52-dang-ky-tai-khoan-va-dang-nhap)
    + [5.3. Tạo project Blynk](#53-tao-project-blynk)
    + [5.4. Thêm các widget vào project](#54-them-cac-widget-vao-project)
  * [6. Cách sử dụng hệ thống](#6-cach-su-dung-he-thong)
    + [6.1. Các chế độ hoạt động](#61-cac-che-do-hoat-dong)
    + [6.2. Điều chỉnh ngưỡng độ ẩm](#62-dieu-chinh-nguong-do-am)
    + [6.3. Hiển thị trạng thái và thời gian](#63-hien-thi-trang-thai-va-thoi-gian)
  * [7. Chức năng các đoạn mã](#7-chuc-nang-cac-doan-ma)
    + [7.1. `setup()`:](#71-setup)
    + [7.2. `loop()`:](#72-loop)
    ...
    + [7.11. `BLYNK_WRITE(VPIN_HUMIDITY_THRESHOLD_HIGH)` và `BLYNK_WRITE(VPIN_HUMIDITY_THRESHOLD_LOW)`:](#711-blynk_writevpin_humidity_threshold_high-va-blynk_writevpin_humidity_threshold_low)
  ...
  * [8. Xử lý sự cố](#8-xu-ly-su-co)
    + [8.1. **ESP8266 không kết nối được WiFi**](#81-esp8266-khong-ket-noi-duoc-wifi)
    + [8.2. **Ứng dụng Blynk không hiển thị đúng thông tin**](#82-ung-dung-blynk-khong-hien-thi-dung-thong-tin)
    + [8.3. **Relay không hoạt động**](#83-relay-khong-hoat-dong)
    + [8.4. **Bơm nước không hoạt động**](#84-bom-nuoc-khong-hoat-dong)
    + [8.5. **Đèn LED cảnh báo không sáng**](#85-den-led-canh-bao-khong-sang)
    + [8.6. **Không nhận được thông báo qua email**](#86-khong-nhan-duoc-thong-bao-qua-email)
    + [8.7. **Không đồng bộ được thời gian thực từ NTP server**](#87-khong-dong-bo-duoc-thoi-gian-thuc-tu-ntp-server)
  * [9. Ghi chú](#9-ghi-chu)
    + [9.1. **An toàn về điện**](#91-an-toan-ve-dien)
    + [9.2. **Bảo trì hệ thống**](#92-bao-tri-he-thong)
    + [9.3. **Tối ưu hóa năng lượng**](#93-toi-uu-hoa-nang-luong)
  * [10. Thông tin liên hệ](#10-thong-tin-lien-he)
